...
# Fixed version with correct JSON extraction
...
# Extract hash from response (handles spacing)
extract_hash() {
    grep "content_hash" /tmp/resp_$$.json | head -1 | sed 's/.*"content_hash"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/'
}

# Extract output file from response
extract_file() {
    grep "output_file" /tmp/resp_$$.json | head -1 | sed 's/.*"output_file"[[:space:]]*:[[:space:]]*"[^"]*\/\([^"]*\)".*/\1/'
}

# Extract cached flag
extract_cached() {
    grep '"cached"' /tmp/resp_$$.json | head -1 | sed 's/.*"cached"[[:space:]]*:[[:space:]]*\([^,}]*\).*/\1/'
}
...
HASH10=$(extract_hash)
CACHED10=$(extract_cached)

info "Request 2: Same HTML 'CACHE_DEMO'"
make_request "CACHE_DEMO"
HASH11=$(extract_hash)
CACHED11=$(extract_cached)
